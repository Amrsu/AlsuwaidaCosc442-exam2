...
public class Creature {
	...
	public void moveBy(int mx, int my, int mz){
		...
		if (mz == -1){
			...
			titleIsStairUp(mz, tile);
		
		Creature other = world.creature(x+mx, y+my, z+mz);
		
		modifyFood(-1);
		
		otherMove(mx, my, mz, tile, other);
	}
	 ...
	 * @param tile
	 */
	private void titleIsStairUp(int mz, Tile tile) {
		if (mz == 1){
	...
	private void otherMove(int mx, int my, int mz, Tile tile, Creature other) {
		if (other == null)
	 ...
	 * @param item
	 */
//	private void actionOfUnequip(Item item) {
//		itemIsArmor(item); 
//			itemIsWeapon(item);
//	}
	 ...
	 * @param item
	 */
	private void itemIsArmor(Item item) {
		if (item == armor){
			if (hp > 0)
	 ...
	 * @param item
	 */
	private void itemIsWeapon(Item item) {
		if (item == weapon) {
			if (hp > 0) 
